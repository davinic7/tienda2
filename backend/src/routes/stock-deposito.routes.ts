...
// POST /api/stock-deposito - Crear o actualizar stock (ADMIN o DEPOSITO)
router.post('/', authenticate, authorize('ADMIN', 'DEPOSITO'), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
// PUT /api/stock-deposito/:id - Actualizar stock (ADMIN o DEPOSITO)
router.put('/:id', authenticate, authorize('ADMIN', 'DEPOSITO'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
